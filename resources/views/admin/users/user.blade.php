        ...
        <div class="flex-1 p-6 bg-gray-100 min-h-screen">
            <div class="bg-white p-6 rounded-2xl shadow">
                ...
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-xl font-semibold">{{ $user ? 'Edit User' : 'Create User' }}</h3>

                    <a href="{{ route('admin.users.index', $tenant) . (count(request()->query()) ? '?' . http_build_query(request()->query()) : '') }}"
                ...
                <!-- EDIT FORM -->
                <form method="POST" action="{{ $user ? route('admin.users.update', [$tenant, $user]) : route('admin.users.store', $tenant) }}" class="space-y-4">
                    @csrf
